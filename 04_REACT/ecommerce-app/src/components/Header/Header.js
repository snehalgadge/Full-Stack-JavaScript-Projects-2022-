      ...
      <div className="w-full bg-amazon_blue text-white px-4 py-3 flex items-center gap-4">
        ...
        <div className="headerHover hidden mdl:inline-flex">
          ...
          <p className="text-sm text-lightText font-light flex flex-col">
            Deliver to
            <span className="font-semibold -mt-1 text-whiteText">Delhi</span>
          </p>
        </div>
        {/* <------------Deliver End here -----------> */}
            ...
            <div ref={ref}>
              <ul className="absolute w-56 h-80 top-10 left-0 overflow-y-scroll overflow-x-hidden bg-white border-[1px] border-amazon_blue text-black p-2 flex-col gap-1 z-50">
                {allItems.map((item) => (
                  <li
                    key={item._id}
                    className="text-sm tracking-wide font-bodyFont border-b-[1px] border-b-transparent hover:border-b-amazon_blue text-black cursor-pointer duration-200"
                  >
                    {item.title}
                  </li>
                ))}
